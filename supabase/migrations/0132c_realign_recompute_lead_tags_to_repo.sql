-- Re-apply recompute_lead_tags() verbatim from 0132b's file text.
--
-- The copy applied first was missing the inline commentary the file carries, so
-- md5(prosrc) and the repo disagreed. Functionally identical, but a file that
-- says one thing while production runs another is how this project has been
-- caught out before, and the hash check is only worth having if it is allowed to
-- fail. Applying the file's exact text is cheaper than deciding "close enough".
--
-- The body is identical to 0132b -- see that file for what the rules mean and
-- why "Never Replied" needs complete history before it is claimed.
